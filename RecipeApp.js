...
const RecipeApp = () => {
  ...
  const addRecipe = () => {
    const newRecipe = {
      ...
      image: "url_to_image", // Замените на реальный URL изображения
    };

    const updatedRecipes = [...defaultRecipes, newRecipe];
    setDefaultRecipes(updatedRecipes);
    setSearchResults(updatedRecipes); // Обновляем результаты поиска
    setNewRecipeName("");
    setNewIngredients("");
    setDescription("");
  };
